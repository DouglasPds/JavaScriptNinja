(function(win, doc) {
    ...
    */
    ...
    //     $inputUsername.value == '' ? (alert('Preencha o nome do usuário!'), cont++) : '';
    //     $inputEmail.value == '' ? (alert('Preencha o e-mail!'), cont++) : '';
    //     $message.value == '' ? (alert('Preencha a mensagem!'), cont++) : ''

    //     if($inputEmail.value != '') {
    //         console.log(isValidEmail($inputEmail.value));
    //         if(isValidEmail($inputEmail.value)){
    //             if(cont === 0) {
    //                 var status = confirm('Tem certeza que deseja enviar o formulário?');
    //                 if(status)
    ...
    */
    function isValidEmail(email) {
        return (email === email.match(/^(\w+[.+]?)+@\w+\.\w{2,}(?:\.\w{2})?$/gm));
    }
})(window, document);